template_name        = "debian-12.10-cloudinit"
template_description = "Base template for Debian 12.10."

# Uncomment if the ISO already exists in the 'iso_storage_pool' location
#iso_file = "debian-12.10.0-amd64-netinst.iso"
# Archive path - current/ serves Debian 13 now.
iso_url      = "https://cdimage.debian.org/cdimage/archive/12.10.0/amd64/iso-cd/debian-12.10.0-amd64-netinst.iso"
iso_checksum = "cb089def0684fd93c9c2fbe45fd16ecc809c949a6fd0c91ee199faefe7d4b82b64658a264a13109d59f1a40ac3080be2f7bd3d8bf3e9cdf509add6d72576a79b"
